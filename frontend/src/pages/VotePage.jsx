...
export default function VotePage({ pollId, navigate }) {
  ...
  async function handleSubmit() {
    ...
    if (res.ok) {
      ...
      setMessage(submitted ? 'Your response has been updated!' : 'Response submitted!')
  ...
  return (
    <Container className="py-4">
      <Button variant="link" className="ps-0 mb-3" onClick={() => navigate('home')}>
        ← Back to Home
      </Button>

      <h4 className="d-inline me-2">{poll.title}</h4>
      <Badge bg={isClosed ? 'secondary' : 'success'} className="mb-2">{poll.status}</Badge>

      {poll.description && <p className="text-muted">{poll.description}</p>}

      {isClosed && (
        <Alert variant="secondary">This poll is closed. You can view your response below.</Alert>
      )}
      {submitted && !isClosed && (
      ...
      {message && <Alert variant={msgVariant} className="py-2">{message}</Alert>}

      <p className="text-muted">Click cells to select your preferred combinations:</p>
      <MatrixGrid rows={poll.rows} columns={poll.columns}
        cells={cells} onChange={setCells} disabled={isClosed} />
